...
import { fetchProjects, addTask } from '../api/taskApi'
...
export default function AddTask() {
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchProjects().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    const newItem = {
      ...
      priority: textFieldVal,
      priceMin: Number(price1),
      priceMax: Number(price2),
      deadline: dateFieldVal
    }

    try {
      const created = await addTask(newItem)
