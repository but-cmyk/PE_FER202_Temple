...
import { addBrand, deleteBrand, fetchProducts } from '../api/productApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageBrands() {
  const { state, dispatch } = useProduct()
  const { brands, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = brands.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addBrand({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
      ...
      const inUse = allItems.some(item => String(item.brandId) === String(targetType.id))
      ...
      await deleteBrand(targetType.id)
        ...
        <tbody>
          {brands.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/brands/${t.id}`}>{t.name}</Link>
              </td>
              <td>
