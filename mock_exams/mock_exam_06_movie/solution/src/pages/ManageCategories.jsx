...
import { addCategory, deleteCategory, fetchMovies } from '../api/movieApi'
...
export default function ManageCategories() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = categories.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addCategory({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchMovies()
      const inUse = allItems.some(item => String(item.categoryId) === String(targetType.id))
      ...
      await deleteCategory(targetType.id)
        ...
        <tbody>
          {categories.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/categories/${t.id}`}>{t.name}</Link>
              </td>
              <td>
