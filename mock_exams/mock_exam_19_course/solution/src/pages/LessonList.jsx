...
export default function LessonList() {
  ...
  const { lessons, modules, loading } = state
  ...
  // Logic Sắp xếp
    const sortedItems = [...lessons].sort((a, b) => {
    switch (sortBy) {
      case 'name-asc':
      ...
      <Row className="mb-3 align-items-center">
        ...
        <Col md={3}>
          {/* TODO-03: Dropdown Form.Select sắp xếp với aria-label="Sort by" */}
            ...
            <option value="num-asc">Duration (min) (Low-High)</option>
            <option value="num-desc">Duration (min) (High-Low)</option>
        ...
        <Col md="auto">
          {/* TODO-01: Chỉ hiện nút Add nếu là Admin */}
                    {isAdmin && (
            <Button onClick={() => navigate('/add')}>+ Add Lesson</Button>
          )}
                            </Col>
      </Row>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {paginatedItems.map((item, idx) => (
            // TODO-01: Truyền canManage={isAdmin} xuống LessonRow
                        <LessonRow
              key={item.id}
              lesson={item}
