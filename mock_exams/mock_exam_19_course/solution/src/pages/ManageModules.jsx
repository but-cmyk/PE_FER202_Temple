...
import { addModule, deleteModule, fetchLessons } from '../api/lessonApi'
...
export default function ManageModules() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = modules.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addModule({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchLessons()
      const inUse = allItems.some(item => String(item.moduleId) === String(targetType.id))
      ...
      await deleteModule(targetType.id)
        ...
        <tbody>
          {modules.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/modules/${t.id}`}>{t.name}</Link>
              </td>
              <td>
