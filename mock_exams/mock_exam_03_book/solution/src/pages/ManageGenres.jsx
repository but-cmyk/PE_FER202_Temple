...
import { addGenre, deleteGenre, fetchBooks } from '../api/bookApi'
...
export default function ManageGenres() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = genres.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addGenre({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchBooks()
      const inUse = allItems.some(item => String(item.genreId) === String(targetType.id))
      ...
      await deleteGenre(targetType.id)
        ...
        <tbody>
          {genres.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/genres/${t.id}`}>{t.name}</Link>
              </td>
              <td>
