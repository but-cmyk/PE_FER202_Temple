...
import { fetchGenres, addBook } from '../api/bookApi'
...
export default function AddBook() {
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchGenres().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    const newItem = {
      name,
      genreId: typeId,
      pages: Number(numFieldVal),
      author: textFieldVal,
      priceMin: Number(price1),
      priceMax: Number(price2),
      publishedDate: dateFieldVal
    }

    try {
      const created = await addBook(newItem)
