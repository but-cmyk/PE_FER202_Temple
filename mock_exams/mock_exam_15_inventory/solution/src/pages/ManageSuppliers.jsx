...
import { addSupplier, deleteSupplier, fetchItems } from '../api/itemApi'
...
export default function ManageSuppliers() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = suppliers.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addSupplier({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
      ...
      const inUse = allItems.some(item => String(item.supplierId) === String(targetType.id))
      ...
      await deleteSupplier(targetType.id)
        ...
        <tbody>
          {suppliers.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/suppliers/${t.id}`}>{t.name}</Link>
              </td>
              <td>
