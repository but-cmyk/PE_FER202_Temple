...
export default function ManageTrainers() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = trainers.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addTrainer({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchGymClasses()
      const inUse = allItems.some(item => String(item.trainerId) === String(targetType.id))
      ...
      await deleteTrainer(targetType.id)
