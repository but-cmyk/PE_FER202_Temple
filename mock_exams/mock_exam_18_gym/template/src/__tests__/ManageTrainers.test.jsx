...
test('TODO-10A: validate không add trùng lặp và add hợp lệ', async () => {
  ...
  expect(api.addTrainer).not.toHaveBeenCalled()
})

test('TODO-10B: check in-use trước khi xóa', async () => {
  api.fetchGymClasses.mockResolvedValue([{ id: '10', name: 'Sub', trainerId: '1' }])
  ...
  expect(api.deleteTrainer).not.toHaveBeenCalled()
})
