...
import { fetchCategories, addGig } from '../api/gigApi'
import { useGig } from '../context/GigContext'

export default function AddGig() {
  const navigate = useNavigate()
  const { dispatch } = useGig()
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchCategories().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const created = await addGig(newItem)
  ...
  return (
    <Container className="mt-4" style={{ maxWidth: '600px' }}>
      <Card>
        <Card.Header as="h5">Add New Gig</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3" controlId="formText">
              <Form.Label>Seller Level</Form.Label>
