...
import * as entityApi from '../api/employeeApi'
...
describe('AuthContext — phiên đăng nhập phải được giữ lại khi reload / truy cập trực tiếp URL', () => {
  beforeEach(() => {
    entityApi.fetchDepartments.mockResolvedValue([{ id: '1', name: 'Standard' }])
    entityApi.fetchEmployees.mockResolvedValue([])
  })

  test('chưa từng đăng nhập → truy cập /departments/999 lần đầu sẽ về trang Login (đúng, không phải bug)', async () => {
    renderApp('/departments/999')
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'Login' })).toBeInTheDocument()
    })
  })

  test('BUG ĐÃ FIX: đăng nhập thành công, sau đó "reload" (remount) tại /departments/999 → vẫn còn phiên đăng nhập, KHÔNG bị đưa về Login', async () => {
    ...
    // hành vi reload trang / dán URL /departments/999 vào thanh địa chỉ.
    unmount()
    renderApp('/departments/999')
    await waitFor(() => {
      expect(screen.queryByRole('heading', { name: 'Login' })).not.toBeInTheDocument()
