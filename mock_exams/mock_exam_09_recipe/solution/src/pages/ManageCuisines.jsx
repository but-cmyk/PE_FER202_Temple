...
import { addCuisine, deleteCuisine, fetchRecipes } from '../api/recipeApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageCuisines() {
  const { state, dispatch } = useRecipe()
  const { cuisines, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = cuisines.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addCuisine({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchRecipes()
      const inUse = allItems.some(item => String(item.cuisineId) === String(targetType.id))
      ...
      await deleteCuisine(targetType.id)
        ...
        <tbody>
          {cuisines.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/cuisines/${t.id}`}>{t.name}</Link>
              </td>
              <td>
