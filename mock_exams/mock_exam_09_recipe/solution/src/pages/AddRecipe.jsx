...
import { fetchCuisines, addRecipe } from '../api/recipeApi'
...
export default function AddRecipe() {
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchCuisines().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    const newItem = {
      name,
      cuisineId: typeId,
      prepTime: Number(numFieldVal),
      difficulty: textFieldVal,
      priceMin: Number(price1),
      priceMax: Number(price2),
      createdDate: dateFieldVal
    }

    try {
      const created = await addRecipe(newItem)
  ...
  return (
    <Container className="mt-4" style={{ maxWidth: '600px' }}>
      <Card>
        <Card.Header as="h5">Add New Recipe</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3" controlId="formPrice1">
              <Form.Label>Cost Min</Form.Label>
            ...
            <Form.Group className="mb-3" controlId="formPrice2">
              <Form.Label>Cost Max</Form.Label>
