...
import { addDoctor, deleteDoctor, fetchAppointments } from '../api/appointmentApi'
...
export default function ManageDoctors() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = doctors.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addDoctor({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchAppointments()
      const inUse = allItems.some(item => String(item.doctorId) === String(targetType.id))
      ...
      await deleteDoctor(targetType.id)
        ...
        <tbody>
          {doctors.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/doctors/${t.id}`}>{t.name}</Link>
              </td>
              <td>
