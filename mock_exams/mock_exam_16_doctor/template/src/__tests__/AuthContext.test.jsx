...
import * as entityApi from '../api/appointmentApi'
...
describe('AuthContext — phiên đăng nhập phải được giữ lại khi reload / truy cập trực tiếp URL', () => {
  beforeEach(() => {
    entityApi.fetchDoctors.mockResolvedValue([{ id: '1', name: 'Standard' }])
    entityApi.fetchAppointments.mockResolvedValue([])
  })

  test('chưa từng đăng nhập → truy cập /doctors/999 lần đầu sẽ về trang Login (đúng, không phải bug)', async () => {
  ...
  test('BUG ĐÃ FIX: đăng nhập thành công, sau đó "reload" (remount) tại /doctors/999 → vẫn còn phiên đăng nhập, KHÔNG bị đưa về Login', async () => {
    ...
    // hành vi reload trang / dán URL /doctors/999 vào thanh địa chỉ.
    unmount()
    renderApp('/doctors/999')
    await waitFor(() => {
      expect(screen.queryByRole('heading', { name: 'Login' })).not.toBeInTheDocument()
