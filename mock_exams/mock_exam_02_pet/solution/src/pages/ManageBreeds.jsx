...
import { addBreed, deleteBreed, fetchPets } from '../api/petApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageBreeds() {
  const { state, dispatch } = usePet()
  const { breeds, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = breeds.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addBreed({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchPets()
      const inUse = allItems.some(item => String(item.breedId) === String(targetType.id))
      ...
      await deleteBreed(targetType.id)
        ...
        <tbody>
          {breeds.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/breeds/${t.id}`}>{t.name}</Link>
              </td>
              <td>
