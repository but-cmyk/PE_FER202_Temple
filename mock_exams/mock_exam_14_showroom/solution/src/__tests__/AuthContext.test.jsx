...
import * as entityApi from '../api/carApi'
...
describe('AuthContext — phiên đăng nhập phải được giữ lại khi reload / truy cập trực tiếp URL', () => {
  beforeEach(() => {
    entityApi.fetchDealers.mockResolvedValue([{ id: '1', name: 'Standard' }])
    entityApi.fetchCars.mockResolvedValue([])
  })

  test('chưa từng đăng nhập → truy cập /dealers/999 lần đầu sẽ về trang Login (đúng, không phải bug)', async () => {
    renderApp('/dealers/999')
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'Login' })).toBeInTheDocument()
    })
  })

  test('BUG ĐÃ FIX: đăng nhập thành công, sau đó "reload" (remount) tại /dealers/999 → vẫn còn phiên đăng nhập, KHÔNG bị đưa về Login', async () => {
    ...
    // hành vi reload trang / dán URL /dealers/999 vào thanh địa chỉ.
    unmount()
    renderApp('/dealers/999')
    await waitFor(() => {
      expect(screen.queryByRole('heading', { name: 'Login' })).not.toBeInTheDocument()
