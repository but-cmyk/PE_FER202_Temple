...
import { addDealer, deleteDealer, fetchCars } from '../api/carApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageDealers() {
  const { state, dispatch } = useCar()
  const { dealers, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = dealers.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addDealer({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchCars()
      const inUse = allItems.some(item => String(item.dealerId) === String(targetType.id))
      ...
      await deleteDealer(targetType.id)
        ...
        <tbody>
          {dealers.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/dealers/${t.id}`}>{t.name}</Link>
              </td>
              <td>
