...
import { addCompany, deleteCompany, fetchJobs } from '../api/jobApi'
...
export default function ManageCompanies() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = companies.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addCompany({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchJobs()
      const inUse = allItems.some(item => String(item.companyId) === String(targetType.id))
      ...
      await deleteCompany(targetType.id)
