...
export default function ManageAgents() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = agents.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addAgent({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchProperties()
      const inUse = allItems.some(item => String(item.agentId) === String(targetType.id))
      ...
      await deleteAgent(targetType.id)
