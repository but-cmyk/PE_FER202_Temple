...
import { addRoomType, deleteRoomType, fetchRooms } from '../api/roomApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageRoomTypes() {
  const { state, dispatch } = useRoom()
  const { roomTypes, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = roomTypes.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addRoomType({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchRooms()
      const inUse = allItems.some(item => String(item.roomTypeId) === String(targetType.id))
      ...
      await deleteRoomType(targetType.id)
        ...
        <tbody>
          {roomTypes.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/roomTypes/${t.id}`}>{t.name}</Link>
              </td>
              <td>
