...
import { addAirline, deleteAirline, fetchFlights } from '../api/flightApi'
import ModalConfirm from '../components/ModalConfirm'

export default function ManageAirlines() {
  const { state, dispatch } = useFlight()
  const { airlines, loading } = state
  ...
  const handleAdd = async (e) => {
    ...
    const exists = airlines.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addAirline({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchFlights()
      const inUse = allItems.some(item => String(item.airlineId) === String(targetType.id))
      ...
      await deleteAirline(targetType.id)
        ...
        <tbody>
          {airlines.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/airlines/${t.id}`}>{t.name}</Link>
              </td>
              <td>
