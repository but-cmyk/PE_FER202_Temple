...
export default function FlightList() {
  ...
  const { flights, airlines, loading } = state
  ...
  // Logic Sắp xếp
    const sortedItems = [...flights].sort((a, b) => {
    switch (sortBy) {
      case 'name-asc':
      ...
      <Row className="mb-3 align-items-center">
        ...
        <Col md="auto">
          {/* TODO-01: Chỉ hiện nút Add nếu là Admin */}
                    {isAdmin && (
            <Button onClick={() => navigate('/add')}>+ Add Flight</Button>
          )}
                            </Col>
      </Row>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {paginatedItems.map((item, idx) => (
            // TODO-01: Truyền canManage={isAdmin} xuống FlightRow
                        <FlightRow
              key={item.id}
              flight={item}
