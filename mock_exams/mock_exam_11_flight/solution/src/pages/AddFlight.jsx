...
import { fetchAirlines, addFlight } from '../api/flightApi'
import { useFlight } from '../context/FlightContext'

export default function AddFlight() {
  const navigate = useNavigate()
  const { dispatch } = useFlight()
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchAirlines().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    const newItem = {
      ...
      aircraft: textFieldVal,
      priceWeekday: Number(price1),
      priceWeekend: Number(price2),
      flightDate: dateFieldVal
    }

    try {
      const created = await addFlight(newItem)
  ...
  return (
    <Container className="mt-4" style={{ maxWidth: '600px' }}>
      <Card>
        <Card.Header as="h5">Add New Flight</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3" controlId="formPrice1">
              <Form.Label>Eco Price</Form.Label>
