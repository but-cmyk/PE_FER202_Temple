...
import { addLocation, deleteLocation, fetchEvents } from '../api/eventApi'
...
export default function ManageLocations() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = locations.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addLocation({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchEvents()
      const inUse = allItems.some(item => String(item.locationId) === String(targetType.id))
      ...
      await deleteLocation(targetType.id)
        ...
        <tbody>
          {locations.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/locations/${t.id}`}>{t.name}</Link>
              </td>
              <td>
