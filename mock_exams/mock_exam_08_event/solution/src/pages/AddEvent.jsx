...
import { fetchLocations, addEvent } from '../api/eventApi'
...
export default function AddEvent() {
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchLocations().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    const newItem = {
      ...
      organizer: textFieldVal,
      priceWeekday: Number(price1),
      priceWeekend: Number(price2),
      eventDate: dateFieldVal
    }

    try {
      const created = await addEvent(newItem)
