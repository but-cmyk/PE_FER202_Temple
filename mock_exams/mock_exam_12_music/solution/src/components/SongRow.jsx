...
export default function SongRow({ song, index, onDelete, canManage }) {
  ...
  return (
    <tr>
      ...
      <td>{song.album}</td>
      <td>
        {formatVND(song.priceWeekday)}
        
              </td>
      <td>
        <div className="d-flex gap-2">
          <Link to={`/songs/${song.id}`} className="btn btn-sm btn-info text-white">
            View
          </Link>
        ...
        {/* TODO-06: ModalConfirm xác nhận xóa */}
          ...
          message={
            <span>
              Are you sure you want to delete <strong>{song.name}</strong>?
            </span>
          }
