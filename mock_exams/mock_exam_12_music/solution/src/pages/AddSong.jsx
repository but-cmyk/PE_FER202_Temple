...
import { fetchArtists, addSong } from '../api/songApi'
...
export default function AddSong() {
  ...
  // TODO-04: fetch danh sách loại thực thể từ API trong useEffect
    useEffect(() => {
    fetchArtists().then(setTypes).catch(console.error)
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const created = await addSong(newItem)
  ...
  return (
    <Container className="mt-4" style={{ maxWidth: '600px' }}>
      <Card>
        <Card.Header as="h5">Add New Song</Card.Header>
        <Card.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3" controlId="formNum">
              <Form.Label>Lượt nghe (k)</Form.Label>
