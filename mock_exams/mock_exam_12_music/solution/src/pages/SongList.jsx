...
export default function SongList() {
  ...
  const { songs, artists, loading } = state
  ...
  // Logic Sắp xếp
    const sortedItems = [...songs].sort((a, b) => {
    switch (sortBy) {
      case 'name-asc':
      ...
      case 'num-asc':
        return Number(a.plays) - Number(b.plays)
      case 'num-desc':
      ...
      <Row className="mb-3 align-items-center">
        ...
        <Col md={3}>
          {/* TODO-03: Dropdown Form.Select sắp xếp với aria-label="Sort by" */}
            ...
            <option value="num-asc">Lượt nghe (k) (Low-High)</option>
            <option value="num-desc">Lượt nghe (k) (High-Low)</option>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {paginatedItems.map((item, idx) => (
            // TODO-01: Truyền canManage={isAdmin} xuống SongRow
                        <SongRow
              key={item.id}
              song={item}
