...
import { addArtist, deleteArtist, fetchSongs } from '../api/songApi'
...
export default function ManageArtists() {
  ...
  const handleAdd = async (e) => {
    ...
    const exists = artists.some(t => t.name.toLowerCase() === typeName.trim().toLowerCase())
    if (exists) {
    ...
    try {
      const created = await addArtist({ name: typeName.trim() })
  ...
  const handleDeleteConfirm = async () => {
    ...
    // TODO-10B: Kiểm tra xem loại danh mục này có đang chứa thực thể con nào không trước khi xóa
        try {
      const allItems = await fetchSongs()
      const inUse = allItems.some(item => String(item.artistId) === String(targetType.id))
      ...
      await deleteArtist(targetType.id)
        ...
        <tbody>
          {artists.map((t, idx) => (
            <tr key={t.id}>
              <td>{idx + 1}</td>
              <td>
                <Link to={`/artists/${t.id}`}>{t.name}</Link>
              </td>
              <td>
